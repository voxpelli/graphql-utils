import { arrayToKeyedObject, pickStringSubsetFromArray, pickSubsetFromArray } from './utils.js';
 ...
 * @typedef {import('./mercurius-types.d.ts').LoaderWithReturn<From, Promise<(To|undefined)[]>>} OneToOneLoader
 */
 ...
 * @param {(ids: string[], info: import('./mercurius-types.d.ts').CustomContext) => Promise<To[]>} loader
...
export async function oneToOneLoader (queries, info, sourceField, loader) {
  ...
  /** @type {(To|undefined)[]} */
  const result = [];
  const byId = arrayToKeyedObject(rows, 'id');
 ...
 * @template {Record<string, any>} From
 * @template {Record<string, any>} To
 * @typedef {import('./mercurius-types.d.ts').LoaderWithReturn<From, Promise<(To[])[]>>} OneToManyLoader
 */
 ...
 * @param {Array<{ obj: From }>} queries
 * @param {import('./mercurius-types.d.ts').CustomContext} info
 * @param {(ids: string[], info: import('./mercurius-types.d.ts').CustomContext) => Promise<{ fromId: string, rows: To[] }[]>} loader
 * @returns {Promise<To[][]>}
 */
export async function oneToManyLoader (queries, info, loader) {
  ...
  /** @type {To[][]} */
  const result = [];
  const byId = arrayToKeyedObject(rows, 'fromId');
